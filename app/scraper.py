...
SOURCE_HOME = "https://www.pakistantoday.com.pk/"
SEARCH_URL = "https://www.pakistantoday.com.pk/search?q={query}"
ARTICLE_PATH_RE = re.compile(r"/\d{4}/\d{2}/\d{2}/[^/]+/?$", re.IGNORECASE)
...
def _normalize_url(url: str) -> str:
    ...
    if parsed.scheme not in {"http", "https"}:
        return ""
    if parsed.netloc not in {"www.pakistantoday.com.pk", "pakistantoday.com.pk"}:
        return ""
    return f"https://www.pakistantoday.com.pk{parsed.path}"


def _is_article_url(url: str) -> bool:
    parsed = urlparse(url)
    if parsed.netloc != "www.pakistantoday.com.pk":
        return False
    if any(
        part in parsed.path
        for part in (
